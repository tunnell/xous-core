...
mod implementation {
    ...
    pub(crate) struct Keyboard {
        ...
        chord_active: u32,
        /// indicate if the chord has been captured. Once captured, further presses are ignored, until all
        /// keys are let up.
        chord_captured: bool,
                ...
                if char_reg & kbd.csr.ms(utra::keyboard::UART_CHAR_STB, 1) != 0 {
                    xous::try_send_message(
                        ...
                        xous::Message::new_scalar(
                            Opcode::InjectKey.to_usize().unwrap(),
                            (char_reg & 0xff) as _,
                            0,
                            0,
                            0,
    ...
    impl Keyboard {
        pub(crate) fn new(sid: xous::SID) -> Keyboard {
            ...
            let default_map = if cfg!(feature = "braille") {
                ...
                // the boot setting is intended to only contain the keyboard mapping...soooooooo.....we manage
                // it as such if feature creep causes more stuff to end up here this should be
                // made into something more generic. But! If you think you need to slap
                // something in the boot setting, think again. This is an unverified,
                // unencrypted and plaintext bit of memory that anyone can read or mess with. The only reason
                // the keyboard layout is kept here is because you want your keyboard to be in
                // the right language before typing in the password to unlock the PDDB, which
                // is *actually* where all the user settings should be located.
        ...
        pub(crate) fn set_chord_interval(&mut self, delay: u32) { self.chord_interval = delay; }
        ...
        pub(crate) fn poll(&mut self) {
            ...
            for r in 0..KBD_ROWS {
                ...
                if cols != 0 {
                    for c in 0..KBD_COLS {
                        if (cols & (1 << c)) != 0 {
                            self.new_state.insert(RowCol { r: r as _, c: c as _ });
                        }
                    }
                }
            }
        ...
        pub(crate) fn track_chord(&mut self, krs: &KeyRawStates) -> Vec<char> {
            ...
            if self.chord_active != 0
                && ((now - self.chord_timestamp) >= self.chord_interval as u64)
                && !self.chord_captured
            {
            ...
            */
            ...
            for rc in krs.keydowns.iter() {
                match self.map {
                    ...
                    _ => {
                        // the rest just have one color of shift
            ...
            for &rc in krs.keyups.iter() {
                match self.map {
                    ...
                    _ => {
                        // the rest just have one color of shift
                        if ((rc.r == 8) && (rc.c == 5)) || ((rc.r == 8) && (rc.c == 9)) {
                            // only set the shift-up if we didn't previously clear it with a double-tap of
                            // shift
                            if self.shift_down {
                                //info!("shift up true");
                                self.shift_up = true;
            ...
            for &rc in krs.keydowns.iter() {
                ...
                {
                    // if there isn't a pre-defined meaning if the key is held *and* it's not the menu key:
                    // it's a repeating key
            ...
            // if we're in a key hold state, we've passed the rate timestamp point, and there's a repeating
            // key defined
        ...
        pub fn is_repeating_key(&self) -> bool { self.repeating_key.is_some() }
    }
}
...
fn main() -> ! {
    ...
    let mut susres = susres::Susres::new(None, &xns, Opcode::SuspendResume as u32, self_cid)
    ...
    loop {
        ...
        match FromPrimitive::from_usize(msg.body.id()) {
            ...
            Some(Opcode::Vibe) => {
                msg_scalar_unpack!(msg, ena, _, _, _, { if ena != 0 { vibe = true } else { vibe = false } })
            }
            ...
            Some(Opcode::SelectKeyMap) => {
                msg_scalar_unpack!(msg, km, _, _, _, { kbd.set_map(KeyMap::from(km)) })
            }
            Some(Opcode::GetKeyMap) => msg_blocking_scalar_unpack!(msg, _, _, _, _, {
                xous::return_scalar(msg.sender, kbd.get_map().into()).expect("can't retrieve keymap");
            }),
            ...
            Some(Opcode::HandlerTrigger) => {
                ...
                if raw_listener_conn.is_some()
                    && raw_listener_op.is_some()
                    && (rawstates.keydowns.len() > 0 || rawstates.keyups.len() > 0)
                {
                    ...
                    let buf = Buffer::into_buf(krs_ser)
                        .or(Err(xous::Error::InternalError))
                        .expect("couldn't serialize krs buffer");
                    buf.lend(raw_listener_conn.unwrap(), raw_listener_op.unwrap())
                        .expect("couldn't send raw scancodes");
                }
                ...
                // interpret scancodes
                // the track_* functions track the keyup/keydowns to modify keys with shift, hold, and chord
                // state
                let kc: Vec<char> = match kbd.get_map() {
                    KeyMap::Braille => kbd.track_chord(&rawstates),
                    _ => kbd.track_keys(&rawstates),
                };
                ...
                // as long as we have a keydown, keep pinging the loop at a high rate. this consumes more
                // power, but keydowns are relatively rare.
