...
pub(crate) fn std_tcp_connect(
    ...
    if let Err(e) = tcp_socket.connect(cx, (address, remote_port), local_port).map_err(|e| match e {
...
pub(crate) fn std_tcp_tx(
    ...
    if !socket.can_send() {
        ...
        insert_or_append(tcp_tx_waiting, WaitingSocket { env: msg, handle: *handle, expiry });
        return;
    }
...
pub(crate) fn std_tcp_rx(
    ...
    let body = match msg.body.memory_message_mut() {
        Some(body) => body,
        None => {
            // Shouldn't fire in normal operation (StdTcpRx is always
            // sent as a memory message); if it does, the IPC contract
            // is broken — log and respond with LibraryError so the
            // caller sees something rather than blocking forever.
            log::warn!(
    ...
    let handle = match our_sockets.get(connection_handle_index) {
        Some(Some(val)) => val,
        _ => {
            // NetError::Invalid surfaces in std as "recv_slice
            // failure" (catch-all, since 4 != 8/9 byte-1 marker).
            // Most likely cause: the underlying TCP connection was
            // force-closed and the handle slot got nulled out, but
            // the std-side `fd` is stale and tries to read anyway.
            log::warn!(
    ...
    if socket.can_recv() {
        ...
        match socket.recv_slice(unsafe { &mut body.buf.as_slice_mut()[..buflen] }) {
            Ok(bytes) => {
                ...
                // so, `None` is abused to represent the value of "0" bytes, which is what is naturally
                // returned as the "error" when you try to create a NonZeroUsize with 0.
    ...
    insert_or_append(tcp_rx_waiting, WaitingSocket { env: msg, handle: *handle, expiry });
}
...
pub(crate) fn std_tcp_peek(
    ...
    if socket.can_recv() {
        ...
        if nonblocking {
            ...
            // Add the message to the TcpRxWaiting list, which will prevent it from getting
            // responded to right away.
            insert_or_append(tcp_peek_waiting, WaitingSocket { env: msg, handle: *handle, expiry });
        }
    }
}
