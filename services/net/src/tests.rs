...
// Precursor PVT2 wlan) instead of the configured 5s. A downstream TLS
// WebSocket client hit this repeatedly during sends: its reader thread
// holds the WebSocket mutex during a 5s read, so the writer was
...
// pre-fix behavior (5-90s). See betrusted-io/xous-core#880.
#[test]
#[cfg_attr(target_env = "sgx", ignore)] // FIXME: https://github.com/fortanix/rust-sgx/issues/31
fn test_read_timeout_quiet_socket_fires_in_bound() {
    ...
    assert!(
        ...
        "read returned far too late: {:?} (configured 500ms; pre-fix would block ~5-90s on \
         ...
         See betrusted-io/xous-core#880.",
        elapsed
    );
