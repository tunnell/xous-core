...
impl PddbMountPoller {
    pub fn new() -> Self {
        ...
        let conn = xns
            .request_connection_blocking(api::SERVER_NAME_PDDB_POLLER)
            .expect("can't connect to Pddb mount poller server");
        PddbMountPoller { conn }
    }

    /// This call is guaranteed to *never* block and return the instantaneous state of the PDDB, even if the
    /// server itself is currently busy processing other requests. This has to be done with a separate
    /// server from the main one, because the main server will block during the mount operations, as it
    /// owns the PDDB data objects and cannot concurrently process the mount check task while manipulating
    /// them. Instead, this routine queries a separate thread that shares an AtomicBool with the main
    /// thread that reports the mount state.
...
pub struct Pddb {
    conn: CID,
    /// a SID that we can directly share with the PDDB server for the purpose of handling key change
    /// callbacks
...
impl Pddb {
    pub fn new() -> Self {
        ...
        let xns = xous_names::XousNames::new().unwrap();
        let conn =
            xns.request_connection_blocking(api::SERVER_NAME_PDDB).expect("can't connect to Pddb server");
        let keys: Arc<Mutex<HashMap<ApiToken, Box<dyn Fn() + 'static + Send>>>> =
    ...
    pub fn is_mounted_blocking(&self) {
        loop {
            let ret = send_message(
                self.conn,
                Message::new_blocking_scalar(Opcode::IsMounted.to_usize().unwrap(), 0, 0, 0, 0),
            )
            .expect("couldn't execute IsMounted query");
            match ret {
    ...
    pub fn mount_attempted_blocking(&self) {
        loop {
            let ret = send_message(
                self.conn,
                Message::new_blocking_scalar(Opcode::MountAttempted.to_usize().unwrap(), 0, 0, 0, 0),
            )
            .expect("couldn't execute IsMounted query");
            match ret {
                xous::Result::Scalar2(code, _count) => {
    ...
    /// This call may cause a password request box to pop up, in the case that the boot PIN is not currently
    /// cached.
    ///
    /// Returns:
    ...
    pub fn try_mount(&self) -> (bool, usize) {
        ...
        .expect("couldn't execute TryMounted query");
        match ret {
    ...
    pub fn try_unmount(&self) -> bool {
        let ret = send_message(
            self.conn,
            Message::new_blocking_scalar(Opcode::TryUnmount.to_usize().unwrap(), 0, 0, 0, 0),
        )
        .expect("couldn't execute TryUnmount query");
        match ret {
    ...
    /// This is a non-blocking message that permanently halts the PDDB server by wedging it in an infinite
    /// loop. It is meant to be called prior to system shutdows or backups, to ensure that other
    /// auto-mounting processes don't undo the shutdown procedure because of an ill-timed "cron" job (the
    /// system doesn't literally have a cron daemon, but it does have the notion of long-running
    /// background jobs that might do something like trigger an NTP update, which would then try to write
    /// the updated time to the PDDB).
    pub fn pddb_halt(&self) {
        send_message(self.conn, Message::new_scalar(Opcode::PddbHalt.to_usize().unwrap(), 0, 0, 0, 0))
            .expect("Couldn't halt the PDDB");
    }
    ...
    pub fn lock_basis(&self, basis_name: &str) -> Result<()> {
        ...
        match ret.code {
            ...
            PddbRequestCode::InternalError => {
                Err(Error::new(ErrorKind::Other, "Internal error closing basis"))
            }
            _ => {
                log::error!("Invalid return code");
                panic!("Invalid return code");
    ...
    pub fn delete_basis(&self, basis_name: &str) -> Result<()> {
        ...
        match ret.code {
            ...
            PddbRequestCode::NotFound => Err(Error::new(ErrorKind::NotFound, "Basis not found")),
            PddbRequestCode::InternalError => {
                Err(Error::new(ErrorKind::Other, "Internal error deleting basis"))
            }
            _ => {
                log::error!("Invalid return code");
                panic!("Invalid return code");
    ...
    /// `alloc_hint` is an optional field to guide the PDDB allocator to put the key in the right pool.
    /// Setting it to `None` is perfectly fine, it just has a potential performance impact, especially for
    /// very large keys. `key_changed_cb` is a static function meant to initiate a message to a server in
    /// case the key in question goes away due to a basis locking.
    pub fn get(
        &self,
        dict_name: &str,
        key_name: &str,
        ...
        let cb_sid = if let Some(sid) = maybe_cb { Some(sid.to_array()) } else { None };
        ...
        match response.result {
            ...
            PddbRequestCode::NotFound => {
                Err(Error::new(ErrorKind::NotFound, "Dictionary or key was not found"))
            }
    ...
    /// deletes a list of keys from a dictionary. The list of keys must be less than MAX_PDDB_DELETE_LEN
    /// characters long.
    pub fn delete_key_list(
        &self,
        dict_name: &str,
        key_list: Vec<String>,
        basis_name: Option<&str>,
        ...
        for keyname in key_list {
            ...
            if keyname.len() + 1 + index < MAX_PDDB_DELETE_LEN {
                ...
                return Err(Error::new(
                    ...
                    "Key list total size exceeds MAX_PDDBKLISTLEN",
                ));
            }
        }
        let mut buf =
        ...
        match response.retcode {
            ...
            ArchivedPddbRetcode::AccessDenied => {
                Err(Error::new(ErrorKind::NotFound, "Dictionary not found, or inaccessible"))
            }
            ArchivedPddbRetcode::Uninit => Err(Error::new(
                ErrorKind::ConnectionAborted,
                "Return code not set processing bulk delete, server aborted?",
            )),
    ...
    pub fn list_keys(&self, dict_name: &str, basis_name: Option<&str>) -> Result<Vec<String>> {
        ...
        match response.code {
            ...
            PddbRequestCode::AccessDenied => {
                return Err(Error::new(ErrorKind::PermissionDenied, "concurrent operation in progress"));
            }
            PddbRequestCode::Uninit => {
                return Err(Error::new(
                    ErrorKind::ConnectionAborted,
                    "Return code not set getting key count, server aborted?",
                ));
            }
        ...
        *self.found_key_count.borrow_mut() = response.found_key_count;
        // v2 key listing packs the key list into a larger [u8] field that should cut down on the number of
        // messages required to list a large dictionary by about a factor of 50.
        ...
        loop {
            ...
            match response.retcode {
                ...
                ArchivedPddbRetcode::AccessDenied => {
                    return Err(Error::new(
                        ...
                        "KeyList facility locked by another process, try again later",
                    ));
                }
                ArchivedPddbRetcode::Uninit => {
                    return Err(Error::new(
                        ErrorKind::ConnectionAborted,
                        "Return code not set fetching list, server aborted?",
                    ));
                }
            ...
            while response.data[index] != 0 && index < MAX_PDDBKLISTLEN {
                ...
                let key = String::from(
                    std::str::from_utf8(&response.data[index..index + strlen]).unwrap_or("UTF8 error"),
                );
    ...
    /// Public function to query efuse security state. Replicated here to avoid exposing RootKeys full API to
    /// the world.
    ...
    /// Reset the "don't ask to init root keys" flag. Used primarily by the OQC test routine to reset this in
    /// case a worker accidentally set it during testing.
    ...
    /// Flush the `SpaceUpdate` journal. SpaceUpdates will leak the last couple hundred or so free space
    /// operations, so periodically flushing this is needed to restore deniability.
    ...
    pub fn manual_prune(&self) {
        send_message(self.conn, Message::new_blocking_scalar(Opcode::Prune.to_usize().unwrap(), 0, 0, 0, 0))
            .expect("couldn't send FlushSpaceUpdate");
    }
    ...
    pub fn rekey_pddb(&self, op: PddbRekeyOp) -> Result<()> {
        let mut buf = Buffer::into_buf(op).or(Err(Error::new(ErrorKind::Other, "Xous internal error")))?;
        buf.lend_mut(self.conn, Opcode::RekeyPddb.to_u32().unwrap())
            .or(Err(Error::new(ErrorKind::Other, "Xous internal error")))
            .map(|_| ())?;
        let result = buf
            .to_original::<PddbRekeyOp, _>()
            .or(Err(Error::new(ErrorKind::Other, "Xous internal error")))?;
        match result {
            ...
            PddbRekeyOp::VerifyFail => {
                Err(Error::new(ErrorKind::InvalidData, "Data verification error while rekeying"))
            }
    ...
    pub fn run_test(&self, args: [u32; 4]) -> Result<(u32, u32)> {
        match send_message(
            ...
            Message::new_blocking_scalar(
                Opcode::InternalTest.to_usize().unwrap(),
                args[0] as usize,
                args[1] as usize,
                args[2] as usize,
                args[3] as usize,
    ...
    pub fn read_dict(
        ...
        */
            ...
            match FromPrimitive::from_u32(header.code).unwrap_or(PddbBulkReadCode::InternalError) {
                ...
                PddbBulkReadCode::Streaming | PddbBulkReadCode::Last => {
                    ...
                    if check_key_index != header.starting_key_index {
                        log::error!(
                            "local key index did not match remote key index: {}, {}",
                            check_key_index,
                            header.starting_key_index
                        );
                    }
                    let mut key_count = 0;
        ...
        if check_total_keys != check_key_index {
            log::error!(
                "Number of keys read does not match expected value: {}, {}",
                check_total_keys,
                check_key_index
            );
        }
...
impl Drop for Pddb {
    fn drop(&mut self) {
        if let Some(cb_sid) = self.cb.take() {
            ...
            send_message(cid, Message::new_blocking_scalar(CbOp::Quit.to_usize().unwrap(), 0, 0, 0, 0))
                .unwrap();
            unsafe {
