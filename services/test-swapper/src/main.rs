...
fn main() -> ! {
    ...
    // NOTE (local swap-test scaffold, not for upstream): the SPI-flash mmap write-back test
    // that used to live here was removed. It exercised xous_swapper::mark_dirty/sync, which were
    // deprecated in 24a70b0eb ("implement writepage mechanism, deprecate sync/dirty"), and it
    // needs a real SPINOR flash backing that renode doesn't provide. The swap-eviction stress
    // loop below is what we actually want: it forces hard-OOM so the swapper evicts pages
    // (including R|X text) and the kernel retrieves them, which is what we're validating.
